...
((window) => {
    ...
    // =================================================================
    // BLOC 2/5 : HANDLERS DE CAPTEURS (API ROBUSTE ET COMPATIBLE)
    // =================================================================
    ...
    const calculateGForces = (ukfState, rawAccels) => {
        ...
        if (typeof math === 'undefined' || !ukf || typeof ukf.quaternionToRotationMatrix !== 'function') {
            // Fallback si UKF/Math non dispo
            return;
        }

        try {
            const q = [ukfState.q_w, ukfState.q_x, ukfState.q_y, ukfState.q_z]; 
            // On vérifie que le quaternion est valide
            if (q.some(isNaN)) return;
    ...
    function updateDashboardDOM(ukfState, isFusionActive) {
        ...
        // --- 4. Position & UKF ---
        if ($('lat-ekf')) $('lat-ekf').textContent = dataOrDefault(ukfState ? ukfState.lat : currentPosition.lat, 6);
        if ($('lon-ekf')) $('lon-ekf').textContent = dataOrDefault(ukfState ? ukfState.lon : currentPosition.lon, 6);
        if ($('alt-ekf')) $('alt-ekf').textContent = formatDistance(ukfState ? ukfState.alt : currentPosition.alt);
        ...
        if (isFusionActive && ukfState) {
            ...
            try {
                if (typeof math !== 'undefined' && ukf && ukf.getStateCovariance) {
...
window.addEventListener('load', () => {
    ...
    if (typeof ProfessionalUKF !== 'undefined' && typeof math !== 'undefined') {
        // L'UKF sera initialisé au premier fix GPS pour avoir la position réelle
    }
    ...
    setInterval(() => {
         ...
         if (!isGpsPaused && ukf && typeof ukf.predict === 'function' && dt_prediction > 0 && ukf.isInitialized()) {
             try {
                 ...
                 // V36: Protection si l'état UKF est corrompu ou incomplet
                 if (ukf.x && ukf.x.size()[0] >= 10) { 
                      const state = ukf.getState(); 
                      // Reconstruction de l'état pour les calculs G-Force et affichage
                      ukfState = { 
                     ...
                     console.warn("⚠️ UKF initialisé mais état incomplet. Utilisation de la vitesse brute.");
                     currentSpeedMs = rawSpeedMs;
                 }
    ...
    setInterval(() => {
        ...
        if (!isGpsPaused && fusionLat !== 0.0) {
            if (weatherUpdateCounter % 60 === 0) { 
                 fetchWeather(fusionLat, currentPosition.lon)
                     .then(data => { 
             ...
             if (typeof updateAstro === 'function') {
                 // updateAstro(fusionLat, currentPosition.lon, fusionAlt, getCDate());
             }
        }
         updatePhysicalState(fusionAlt, fusionLat); 
    }, 1000); 
