// =================================================================
// GNSS SPACETIME DASHBOARD - FICHIER FINAL PROFESSIONNEL (V33)
// CORRECTION CRITIQUE V33: Structure optimisée pour la Robustesse et la Compatibilité. 
// Définition des handlers GPS/IMU en tête de script pour prévenir les blocages sur navigateur.
// =================================================================
...
((window) => {
    ...
    let rawSpeedMs = 0.0;       
    let curAcc = {x:0, y:0, z:G_ACC_STD}, curGyro = {x:0, y:0, z:0}; // Accel Z initialisé à la gravité
    ...
    // Utilitaires (dataOrDefault, formatDistance, etc. - non modifiés, omis pour concision)
    ...
    /** Placeholder pour la fonction Météo (Nécessaire pour ne pas crasher) */
    ...
    // (Fonctions updatePhysicalState, calculateGForces, updateRelativityAndForces non modifiées, omises pour concision)
    
    const calculateBarometricAltitude = (P_hPa, T_C) => { /* ... */ return 0.0; };
    const getGravity = (latRad, alt) => { /* ... */ return G_ACC_STD; };
    ...
    const updatePhysicalState = (fusionAlt, fusionLat) => {
        ...
        const currentSpeedOfSound = Math.sqrt(GAMMA * R_AIR * T_K);
    ...
    // (Fonctions updateSpiritLevel, updateDashboardDOM, updateTimeCounters non modifiées, omises pour concision)
    ...
    function setupEventListeners() {
        ...
        // Événement pour le bouton TOUT RÉINITIALISER
...
window.addEventListener('load', () => {
    ...
    if (typeof ProfessionalUKF !== 'undefined') {
        // Initialisation de l'UKF à 0, il sera initialisé correctement au premier fix GPS
    ...
    setInterval(() => {
         ...
         if (!isGpsPaused && ukf && typeof ukf.predict === 'function' && dt_prediction > 0 && ukf.isInitialized()) {
             try {
                 // Prédire l'état en utilisant les données IMU brutes (acc et gyro)
                 ...
                 // Détection de ZUUV (Zero-Velocity Update)
                 ...
                 console.error("🔴 ERREUR UKF PREDICT. Arrêt de la fusion.", e);
                 currentSpeedMs = rawSpeedMs; 
