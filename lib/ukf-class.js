// =================================================================
// PROFESSIONAL UNSCENTED KALMAN FILTER (UKF) - 21 ÉTATS - VERSION QUATERNION (V30)
// CORRECTION V30: Support de l'intégration 3D (Position/Distance) par l'IMU/UKF à 50Hz.
...
class ProfessionalUKF {
    constructor(lat = 0, lon = 0, alt = 0) {
        ...
        // --- TUNING DES MATRICES (V30) ---
        
        // P: Incertitude Initiale
        this.P = math.diag(math.flatten(math.ones(this.n).map(val => val * 10.0))); 

        // Q: Bruit de Processus (Légèrement augmenté pour laisser l'IMU jouer)
        this.Q = math.diag(new Array(this.n).fill(1e-5)); 
        this.Q.subset(math.index(10, 10), 1e-6); // Gyro Bias
        this.Q.subset(math.index(13, 13), 1e-5); // Accel Bias

        // R_GPS: Bruit de Mesure GPS (Augmenté pour ne pas "noyer" l'IMU)
        this.R_GPS_BASE = math.diag([25.0, 25.0, 25.0, 1.0]); // [Lat, Lon, Alt, Speed]

        // R_ZUUV: Mesure très stricte pour l'arrêt (ZUPT)
        this.R_ZUUV = math.diag([1e-4, 1e-4, 1e-4, 1e-5, 1e-5, 1e-5]); // Vitesse (3) et Gyro Bias (3)
    ...
    quaternionToEuler(q) {
        ...
        const yaw = Math.atan2(2*(w*z + x*y), 1 - 2*(y*z + x*y));
    ...
    stateTransitionFunction(x_sigma, dt, rawAccels, rawGyros) {
        ...
        const f_ltf = math.multiply(R_mat, acc_corr); // Force dans le repère local
        ...
        x_new.subset(math.index(0,0), lat + d_lat * this.R2D);
        x_new.subset(math.index(1,0), x_sigma.subset(math.index(1, 0)) + d_lon * this.R2D); // Lon
        x_new.subset(math.index(2,0), alt + d_alt);
        ...
        // Markov Bias & Clock (restent pour l'intégrité de l'état 21)
        x_new.subset(math.index(10,0), gyroBias.subset(math.index(0,0)) * (1.0 - dt/this.TAU_GYRO));
        x_new.subset(math.index(13,0), accelBias.subset(math.index(0,0)) * (1.0 - dt/this.TAU_ACCEL));
    ...
    predict(dt, rawAccels, rawGyros) {
        ...
        // UKF Prediction steps (Generate, Propagate, Reconstruct x_bar and P_bar)
    ...
    // GPS Update (4 mesures: Lat, Lon, Alt, Vitesse totale)
    h_GPS(x) {
    ...
    update(pos) { 
        ...
        // La matrice R est ajustée dynamiquement en fonction de la précision GPS réelle
    ...
    // ZUUV Update (Zero Velocity Update - 6 mesures)
    h_ZUUV(x) {
