/**
 * PROFESSIONAL UKF V70 - ARMORED MOMENTUM EDITION
 * - Conservation de l'inertie (Newton)
 * - Suspension numérique (Filtre anti-pavés)
 * - Limiteur de Slew-Rate (Anti-ralentisseurs/chocs)
 * - Intégration de Verlet à 50Hz
 */
...
class ProfessionalUKF {
    constructor(lat = 48.8566, lon = 2.3522, alt = 0) {
        ...
        // --- FILTRES DE PROTECTION (NOUVEAU) ---
        this.accelSmooth = { x: 0, y: 0, z: 0 };
        this.lpf = 0.12;          // Coefficient d'absorption des vibrations (pavés)
        this.maxHumanAcc = 18.0;  // Seuil anti-choc (Rejette les pics > 1.8g)
    ...
    predict(dt, accRaw, gyroRaw) {
        ...
        // On empêche les ralentisseurs de simuler une vitesse de 100 km/h
        const limit = (v) => Math.abs(v) > this.maxHumanAcc ? (this.maxHumanAcc * Math.sign(v)) : v;
        const cleanAcc = {
            x: limit(accRaw.x),
            y: limit(accRaw.y),
            z: limit(accRaw.z)
        ...
        // 2. SUSPENSION NUMÉRIQUE (Absorption des pavés)
        this.accelSmooth.x += this.lpf * (cleanAcc.x - this.accelSmooth.x);
        this.accelSmooth.y += this.lpf * (cleanAcc.y - this.accelSmooth.y);
        this.accelSmooth.z += this.lpf * (cleanAcc.z - this.accelSmooth.z);

        // 3. CORRECTION MATÉRIELLE (BIAIS)
        const ax = this.accelSmooth.x - this.bias.ax;
        const ay = this.accelSmooth.y - this.bias.ay;
        const az = this.accelSmooth.z - (this.bias.az - 9.80665);

        // 4. ROTATION 3D (QUATERNIONS)
        const q = [this.x.get([6,0]), this.x.get([7,0]), this.x.get([8,0]), this.x.get([9,0])];
        const accWorld = this.rotateVector(q, [ax, ay, az]);
        accWorld[2] += 9.80665; 

        // 5. INTÉGRATION DE VERLET (Maintien de la vitesse réelle)
        let vx = this.x.get([3, 0]);
        let vy = this.x.get([4, 0]);
        let vz = this.x.get([5, 0]);

        // On ignore les micro-bruits de capteur résiduels sous 0.01 m/s²
        if (Math.sqrt(accWorld[0]**2 + accWorld[1]**2 + accWorld[2]**2) > 0.01) {
            vx += (this.lastAccWorld[0] + accWorld[0]) * 0.5 * dt;
            vy += (this.lastAccWorld[1] + accWorld[1]) * 0.5 * dt;
            vz += (this.lastAccWorld[2] + accWorld[2]) * 0.5 * dt;
        }

        // 6. CONSERVATION DU MOMENTUM & FRICTION RÉALISTE
