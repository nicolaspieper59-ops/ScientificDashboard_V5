// =================================================================
// PROFESSIONAL UNSCENTED KALMAN FILTER (UKF) - 21 ÉTATS - VERSION FINALE (100%)
...
class ProfessionalUKF {
    constructor() {
        ...
        this.G_E = 9.780327;          // Gravité à l'équateur
        this.R_MAJOR = 6378137.0;     // Rayon majeur WGS84 (m)
        ...
        this.TAU_GYRO_BIAS = 3600;  // Constante de temps Markov pour le biais Gyro
        this.TAU_ACCEL_BIAS = 7200; // Constante de temps Markov pour le biais Accel
        this.B_REF_LTF = math.matrix([[0], [10], [45]]); // Champ Mag de référence LTF [Nord, Est, Bas] (Doit être calibré)
    }
    ...
    getWGS84Parameters(latRad, alt) {
        ...
        // Formule de la gravité locale
        const g_0 = this.G_E * (1 + 0.0053024 * sin_lat * sin_lat);
        const g_mag = g_0 - 3.086e-6 * alt; // Correction d'altitude (anomalie à l'air libre)
        
        // Vecteur de gravité dans le LTF (Down)
    ...
    quaternionIntegrate(q, w, dt) {
        ...
        // Quaternion de taux (Delta Q)
        const rate_q = [
            cos_angle, 
            (w[0] / norm_w) * sin_angle, 
            (w[1] / norm_w) * sin_angle, 
            (w[2] / norm_w) * sin_angle
        ];
    ...
    /** Génère les Sigma Points à partir de l'état (x) et de la covariance (P) */
    ...
    stateTransitionFunction(x_sigma, dt, rawAccels, rawGyros) {
        ...
        // Accélération nette dans le LTF (Force - Gravité)
        const accel_LTF = math.subtract(force_LTF, g_vector);
        
        // Intégration Pos (Lat/Lon/Alt) - Formule cinématique LTF
        ...
        // Intégration Vel (Vn, Ve, Vd) - Euler
        ...
        // Modèle de l'horloge (intégré à la main ici pour la simplicité Euler)
    ...
    predict(dt, rawAccels, rawGyros) { 
        ...
        // Reconstruire l'état prédit (x_bar)
        ...
        // Reconstruire la matrice de Covariance prédite (P_bar)
    ...
    UKF_Update_Core(m, R, y_measurement, measurementFunction) {
        ...
        // Calculer Pxy (Covariance Croisée)
        ...
        // Calculer le Gain de Kalman (K)
        const K = math.multiply(Pxy, math.inv(Pyy)); 
        
        // CORRIGER L'ÉTAT et la COVARIANCE
        ...
        // Maintien de l'état du Quaternion
        this.normalizeQuaternion(this.x);
    }
    ...
    measurementFunctionGPS(x_sigma) {
        ...
        // Mesure observée: [Lat, Lon, Alt, Vitesse]
    ...
    update(pos) { // GPS
        ...
        // Lissage du reset du Clock Bias pour éviter les sauts
    ...
    measurementFunctionZUUV(x_sigma) {
        ...
        const Vy = x_sigma.subset(math.index(4, 0));
        const Vz = x_sigma.subset(math.index(5, 0));
        // Observation des Biais Gyro pour forcer leur correction (d'où le ZUUV)
    ...
    updateZUUV() {
        if (!this.initialized) return;
        const m = 6;
        const y_zuuv = math.matrix(math.zeros([m, 1])); // Mesure de vitesse et Bias Gyro nuls
    ...
    measurementFunctionMag(x_sigma) {
        ...
        // Projection du champ de référence LTF vers le Body Frame prédit
    ...
    measurementFunctionBaro(x_sigma) {
        // La mesure barométrique n'observe que l'altitude
