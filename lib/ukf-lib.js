...
// CORRECTION LOGIQUE CRITIQUE: Pitch/Roll retournés en radians.
// =================================================================

class ProfessionalUKF {
    constructor() {
        // VÉRIFICATION CRITIQUE: Si math.js n'est pas chargé, le script s'arrête ici.
        ...
        this.Q.subset(math.index(11, 11), 1e-6); 
        this.Q.subset(math.index(12, 12), 1e-6); 
        this.Q.subset(math.index(13, 13), 1e-5); // Accel Bias X
        this.Q.subset(math.index(14, 14), 1e-5); 
        this.Q.subset(math.index(15, 15), 1e-5); 

        // R_GPS: Bruit de Mesure GPS (25m² = 5m standard déviation)
        this.R_GPS = math.diag([25.0, 25.0, 36.0, 2.0]); // [Lat, Lon, Alt, Speed]

        // R_ZUUV: Mesure très stricte pour l'arrêt
    ...
    // -------------------------------------------------------------------------
    // FONCTIONS UTILITAIRES MATHÉMATIQUES (NÉCESSAIRES ET COMPLÈTES)
    // -------------------------------------------------------------------------
    ...
    getState() {
        ...
        return {
            ...
            vel_D: Vz,
            // 🛑 CORRECTION CRITIQUE: On retourne les angles en radians, le script principal fera la conversion en degrés pour l'affichage.
            pitch: euler.pitch, 
            roll: euler.roll, 
            // Yaw reste en degrés pour une utilisation simple dans le script principal
